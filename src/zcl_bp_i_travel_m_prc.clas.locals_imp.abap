...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE zi_travel_m_prc.
...
********************************************************************************
  METHOD get_features.
    ...
    READ ENTITY zi_travel_m_prc FROM VALUE #( FOR keyval IN keys
                                                      (  %key                    = keyval-%key
  ...
  METHOD validate_agency.
    ...
    READ ENTITY zi_travel_m_prc\\travel FROM VALUE #(
        FOR <root_key> IN keys ( %key-mykey     = <root_key>-mykey
                                %control = VALUE #( agency_id = if_abap_behv=>mk-on ) ) )
        RESULT DATA(lt_travel).

    DATA lt_agency TYPE SORTED TABLE OF /DMO/I_Agency WITH UNIQUE KEY AgencyID.
  ...
  METHOD calculatetravelkey.
    READ ENTITIES OF zi_travel_m_prc IN LOCAL MODE
        ENTITY Travel
          FIELDS ( travel_id )
    ...
    "Get max travelID
    SELECT SINGLE FROM ztravel_app_prc FIELDS MAX( travel_id ) INTO @DATA(lv_max_travelid).

    "update involved instances
    MODIFY ENTITIES OF zi_travel_m_prc IN LOCAL MODE
      ENTITY Travel
